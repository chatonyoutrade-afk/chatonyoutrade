...
export default function KycStatusPage() {
  ...
  const [startingDigiLocker, setStartingDigiLocker] = useState(false);
  ...
  return <main className="kyc-status-shell">
    ...
    <section className={`kyc-status-hero ${content.tone}`}><div className="kyc-status-orb"><i>{content.icon}</i></div><span>{content.eyebrow}</span><h1>{content.title}</h1><p>{content.description}</p><em>{content.badge}</em>{error ? <p className="kyc-error" role="alert">{error}</p> : null}<button type="button" onClick={primaryAction} disabled={startingDigiLocker}>{application.status === "pending" && digiLocker !== "completed" ? (startingDigiLocker ? "Opening DigiLocker…" : digiLocker === "pending" ? "Resume DigiLocker consent" : "Connect DigiLocker") : content.action} <b>→</b></button></section>
