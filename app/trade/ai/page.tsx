...
export default function AiDashboard(){
 ...
 const [decisions,setDecisions]=useState(demoDecisions),[trades,setTrades]=useState(demoTrades),[balance,setBalance]=useState(10000),[starting,setStarting]=useState(10000);
 useEffect(()=>{Promise.all([fetch("/api/account",{cache:"no-store"}).then(r=>r.json()),fetch("/api/decisions",{cache:"no-store"}).then(r=>r.json())]).then(([accountData,decisionData])=>{if(accountData.account){setStarting(accountData.account.startingBalance);setBalance(accountData.account.startingBalance+(accountData.trades||[]).filter((item:{status:string})=>item.status==="closed").reduce((sum:number,item:{pnlPaise:number})=>sum+item.pnlPaise/100,0));const saved=(accountData.trades||[]).filter((item:{status:string})=>item.status==="closed").slice(0,20).map((item:{asset:string;side:string;entryPrice:number;exitPrice:number;pnlPaise:number})=>({coin:`${item.asset}/USDT`,side:item.side,entry:`$${item.entryPrice.toLocaleString("en-US")}`,exit:`$${item.exitPrice?.toLocaleString("en-US")}`,pnl:`${item.pnlPaise>=0?"+":"-"}₹${Math.abs(item.pnlPaise/100).toFixed(2)}`,result:item.pnlPaise>=0?"Win":"Loss"}));if(saved.length)setTrades(saved)}const savedDecisions=(decisionData.decisions||[]).slice(0,30).map((item:{createdAt:number;asset:string;decision:string;confidence:number;reasons:string[]})=>({time:new Date(item.createdAt).toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"}),coin:item.asset,type:item.decision,confidence:`${item.confidence}%`,reason:item.reasons[0]||"Quant conditions evaluated",tone:item.decision==="BUY"?"buy":item.decision==="SELL"?"sell":"neutral"}));if(savedDecisions.length)setDecisions(savedDecisions)}).catch(()=>{})},[]);
  ...
  <div className="ai-dash-layout">
   ...
   <section className="ai-dash-main">
    ...
    <div className="ai-metrics">
     ...
     <article><span>MAX DRAWDOWN</span><strong>-2.8%</strong><small>Within 3% daily limit</small><i className="metric-icon safe">◇</i></article>
    </div>

    <div className="ai-overview-grid" id="performance">
     <article className="equity-card"><header><div><span>EQUITY CURVE</span><b>₹10,842</b></div><small>{range} performance</small></header><div className="equity-chart"><div className="equity-fill"/><div className="equity-line"/><i className="point p1"/><i className="point p2"/><i className="point p3"/><b>+8.42%</b><span>₹10,000</span></div><footer><span>Jul 22</span><span>Aug 01</span><span>Aug 10</span><span>Today</span></footer></article>
     <article className="ai-live-card"><header><span><i/> QUANT STATUS</span><small>Live</small></header><div className="brain-orbit"><i>✦</i><b/><b/><b/></div><h2><a href="/trade/markets">Watching 4 crypto markets →</a></h2><p>Signals refresh from public candles every <b>15s</b></p><div className="last-decision"><span>LAST SAVED DECISION</span><strong>{lastDecision?.type||"SCANNING"}</strong><small>{lastDecision?`${lastDecision.coin} · ${lastDecision.confidence} · ${lastDecision.reason}`:"Waiting for live quant analysis."}</small></div><button className={auto?"on":""} onClick={()=>window.location.href="/trade/auto"}><span><b>AI Auto Mode</b><small>{auto?"Active within risk rules":"Set limits before activation"}</small></span><i/></button></article>
    </div>
