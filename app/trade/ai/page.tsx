  ...
  <div className="ai-dash-layout">
   ...
   <section className="ai-dash-main">
    ...
    <div className="ai-overview-grid" id="performance">
     ...
     <article className="ai-live-card"><header><span><i/> QUANT STATUS</span><small>{watchedCoins.length?"Live":"Paused"}</small></header><div className="brain-orbit"><i>✦</i><b/><b/><b/></div><h2><a href={watchedCoins.length?"/trade/markets":"/trade/bots"}>{watchedCoins.length?`Watching ${watchedCoins.join(" + ")} →`:"Create a paper bot →"}</a></h2><p>Signals refresh from public candles every <b>15s</b></p><div className="last-decision"><span>LAST SAVED DECISION</span><strong>{lastDecision?.type||"SCANNING"}</strong><small>{lastDecision?`${lastDecision.coin} · ${lastDecision.confidence} · ${lastDecision.reason}`:watchedCoins.length?`Waiting for ${watchedLabel} quant analysis.`:"Activate a paper bot to start monitoring."}</small></div><button className={auto?"on":""} onClick={()=>window.location.href="/trade/auto"}><span><b>AI Auto Mode</b><small>{auto?"Active within risk rules":"Set limits before activation"}</small></span><i/></button></article>
    </div>
