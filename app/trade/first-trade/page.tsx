...
type LiveSignal = { signal: "BUY" | "SELL" | "NO TRADE"; confidence: number; entry: number; stopLoss: number; takeProfit: number; generatedAt: number; reasons?: string[] };
...
export default function FirstTradePage() {
  ...
  const entry = signal?.entry ?? market.price;
  const stop = signal?.stopLoss ?? entry * 0.988;
  const target = signal?.takeProfit ?? entry * 1.03;
  const plannedRisk = amount * Math.abs(entry - stop) / entry;
  const eligible = Boolean(signal && signal.signal !== "NO TRADE" && signal.confidence >= 80);

  useEffect(() => {
    let active = true;
    setSignalLoading(true); setSignal(null); setError("");
    fetch(`/api/signals?asset=${encodeURIComponent(asset)}`, { cache: "no-store" })
      .then(async (response) => { const data = await response.json(); if (!response.ok) throw new Error(data.error || "Live signal unavailable"); return data as LiveSignal; })
      .then((data) => { if (active) setSignal(data); })
      .catch((reason) => { if (active) setError(reason instanceof Error ? reason.message : "Live signal unavailable"); })
  ...
  const execute = async () => {
    ...
    if (!signal || !eligible || signal.signal === "NO TRADE") { setError("A fresh BUY or SELL signal with at least 80% confidence is required."); return; }
    setExecuting(true); setError("");
    try {
      const response = await fetch("/api/trades", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ asset, side: signal.signal, amount, entryPrice: entry, stopPrice: stop, targetPrice: target, signalGeneratedAt: signal.generatedAt }) });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || "Risk engine blocked this paper order");
      setStep(4);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Paper order could not be saved"); }
  ...
  return <main className="first-trade-shell">
    ...
    <section className="first-trade-card">
      ...
      {step===1?<div className="first-step"><header><span>STEP 2 OF 5 · AI SIGNAL</span><h1>Understand the decision.</h1><p>AI explains its live view before you choose whether to continue.</p></header><div className="first-signal"><div><span>LIVE PAPER SIGNAL</span><strong>{signalLoading ? "SCANNING" : signal?.signal ?? "UNAVAILABLE"}</strong><p>{asset}/USDT</p></div><div className="first-confidence"><b>{signal?.confidence ?? 0}<small>%</small></b><span><i/></span><small>{eligible ? "ELIGIBLE" : "WAIT"}</small></div></div><div className="first-reasons">{(signal?.reasons?.slice(0,3) ?? ["Waiting for fresh Binance candle analysis."]).map((reason,index)=><article key={reason}><i>{eligible?"✓":"◇"}</i><span><b>{["Trend","Momentum","Volume"][index] ?? "Signal"}</b><small>{reason}</small></span></article>)}</div><div className="first-warning"><i>!</i><p><b>{eligible ? "AI can be wrong." : "No paper entry yet."}</b><span>{eligible ? "A confidence score is not a guarantee. Risk limits still apply." : "Continue unlocks only when the live direction is BUY or SELL with at least 80% confidence."}</span></p></div></div>:null}
      {step===2?<div className="first-step"><header><span>STEP 3 OF 5 · AMOUNT &amp; RISK</span><h1>Control the downside.</h1><p>Choose a paper allocation. Stop-loss and take-profit are attached before entry.</p></header><div className="first-amount"><label>Paper allocation<span>₹<input type="number" min="100" max="5000" value={amount} onChange={event=>setAmount(Number(event.target.value))}/></span></label><nav>{[250,500,1000,2000].map(value=><button key={value} className={amount===value?"active":""} onClick={()=>setAmount(value)}>₹{value.toLocaleString("en-IN")}</button>)}</nav></div><div className="first-levels"><article><small>ENTRY PRICE</small><b>${entry.toLocaleString("en-US")}</b></article><article><small>STOP-LOSS</small><b>${stop.toLocaleString("en-US",{maximumFractionDigits:2})}</b></article><article><small>TAKE-PROFIT</small><b>${target.toLocaleString("en-US",{maximumFractionDigits:2})}</b></article></div><div className="first-risk-check"><i>✓</i><span><b>Risk preview</b><small>Planned loss ₹{plannedRisk.toFixed(2)} · final server validation runs at execution.</small></span></div></div>:null}
      {step===3?<div className="first-step"><header><span>STEP 4 OF 5 · REVIEW ORDER</span><h1>Confirm your paper trade.</h1><p>Review every value before executing the simulated order.</p></header><div className="first-review"><header><i style={{background:market.color}}>{asset[0]}</i><span><b>{asset}/USDT</b><small>Paper market order</small></span><em>{signal?.signal}</em></header><div><span><small>Allocation</small><b>₹{amount.toLocaleString("en-IN")}</b></span><span><small>Entry</small><b>${entry.toLocaleString("en-US")}</b></span><span><small>Stop-loss</small><b>${stop.toLocaleString("en-US",{maximumFractionDigits:2})}</b></span><span><small>Take-profit</small><b>${target.toLocaleString("en-US",{maximumFractionDigits:2})}</b></span></div></div><div className="first-checks"><span><i>✓</i>Paper balance available</span><span><i>✓</i>AI confidence above 80%</span><span><i>✓</i>Position limit available</span><span><i>✓</i>Stop-loss attached</span></div><p className="first-disclaimer">This is a guided simulated order. It does not move real money or place an exchange trade.</p></div>:null}
      {step===4?<div className="first-complete"><div>✓</div><span>FIRST PAPER TRADE COMPLETE</span><h1>Your {asset} position<br/><em>is now being tracked.</em></h1><p>The demo position is ready for portfolio monitoring. Continue to the terminal to view market movement, P&amp;L and protective levels.</p><section><article><small>POSITION</small><b>{asset}/USDT · {signal?.signal}</b></article><article><small>ALLOCATION</small><b>₹{amount.toLocaleString("en-IN")}</b></article><article><small>MAX PLANNED LOSS</small><b>₹{plannedRisk.toFixed(2)}</b></article></section><nav><a href="/trade/portfolio">View portfolio <b>→</b></a><a href="/trade">Open trading terminal</a></nav><small>Guided paper result · No real funds used</small></div>:null}
      {error?<p className="first-error" role="alert">{error}</p>:null}
      {step<4?<footer className="first-actions"><button onClick={back} disabled={step===0||executing}>← Back</button><span>Paper balance · ₹10,000</span>{step<3?<button className="primary" onClick={next} disabled={step===1&&!eligible}>Continue <i>→</i></button>:<button className="primary" onClick={execute} disabled={executing||!eligible}>{executing?"Saving paper order…":"Execute paper trade"} <i>↗</i></button>}</footer>:null}
    </section>
  </main>;
}
