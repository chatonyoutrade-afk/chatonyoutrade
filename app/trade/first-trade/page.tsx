...
export default function FirstTradePage() {
  ...
  const eligible = signal?.signal === "BUY" && signal.confidence >= 80;
  ...
  const execute = async () => {
    ...
    if (!signal || !eligible) { setError("A fresh BUY signal with at least 80% confidence is required."); return; }
    setExecuting(true); setError("");
    try {
      const response = await fetch("/api/trades", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ asset, side: "BUY", amount, entryPrice: entry, stopPrice: stop, targetPrice: target, signalGeneratedAt: signal.generatedAt }) });
