...
import { exchangeConnections, kycApplications, ownerSecurity, testnetOrders } from "../../../db/schema";
...
export async function GET() {
  ...
  const db = getDb();
  const [{ settings }, applications, connections, securityRows, orders] = await Promise.all([
  ...
  const checks = [
    ...
    { id: "owner", title: "Email two-step owner verification", detail: security?.enabled && security.verifiedAt ? `Enabled ${new Date(security.verifiedAt).toLocaleDateString("en-IN")}` : "Verify account-email possession after password sign-in", passed: Boolean(security?.enabled), href: "/trade/profile?tab=security", action: "Set up verification", category: "technical" },
