...
export default function Login(){
 ...
 const submit=async(event:FormEvent)=>{
  ...
  if(view==="forgot"){setView("sent");return}
  ...
  <section className="login-layout">
   ...
   <section className="login-panel">
    <div className="login-card">
     {view==="sent"?<div className="email-sent"><i>◇</i><span>PASSWORD RESET</span><h2>Reset is not available yet.</h2><p>Password reset needs an email service, which is not connected on this deployment. Contact support to regain access to <b>{email}</b>.</p><button onClick={()=>setView("login")}>Back to sign in</button></div>:<>
      <header><span>{view==="signup"?"CREATE PAPER ACCOUNT":view==="forgot"?"ACCOUNT RECOVERY":"WELCOME BACK"}</span><h2>{view==="signup"?"Start trading safely.":view==="forgot"?"Reset your password.":"Sign in to Trade."}</h2><p>{view==="signup"?"Set up your virtual balance and risk rules in minutes.":view==="forgot"?"Password reset is not connected on this deployment yet.":"Continue to your AI paper-trading workspace."}</p></header>
      <form onSubmit={submit}>
       ...
       <button className="login-submit" disabled={loading||(view==="signup"&&!agree)}>{loading?<><i/>{view==="signup"?"Creating account…":"Signing in…"}</>:<>{view==="signup"?"Create account":view==="forgot"?"Continue":"Sign in"}<span>→</span></>}</button>
      </form>
